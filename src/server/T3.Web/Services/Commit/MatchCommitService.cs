...
public interface IMatchCommitService
{
    Task<IEnumerable<MatchCommitEntity>> GetAll(MatchId matchId);
    Task<MatchCommitEntity> GetById(CommitId commitId);
    Task Add(MatchCommitEntity commit);
}

public class MatchCommitService : IMatchCommitService
{
    private readonly IMongoCollection<MatchCommitEntity> _collection;
    private readonly ILogger<MatchCommitService> _logger;
    private readonly IAccountPublicKeyService _accountPublicKeyService;
    private readonly ITimestampService _timestampService;

    public MatchCommitService(
        ILogger<MatchCommitService> logger,
        IMongoCollection<MatchCommitEntity> collection,
    ...
    public async Task<IEnumerable<MatchCommitEntity>> GetAll(MatchId matchId)
    {
        return await _collection.Find(x => x.Content.Header.MatchId.Value == matchId.Value).ToListAsync();
    }

    public async Task<MatchCommitEntity> GetById(CommitId commitId)
    {
        return await _collection.Find(x => x.Content.Header.CommitId.Value == commitId.Value).SingleAsync();
    }

    public async Task Add(MatchCommitEntity commit)
    {
    ...
    private async Task ValidateTime(MatchCommit commit)
    {
    ...
    private async Task ValidateAuthor(MatchCommitEntity commit)
    {
    ...
    private void ValidatePayload(MatchCommitEntity commit)
    {
    ...
    private void ValidateSignature(MatchCommitEntity message)
    {
